...
const productController = {
  ...
  async getProductCategoryById(req, res) {
    try {
      const category = await productService.getProductCategoryById(req.params.categoryId, req.user.farm_id)
      if (!category) {
  ...
  async createProductCategory(req, res) {
    try {
      ...
      await auditLogService.logActivity(
        ...
        { categoryName, note },
        auditLogService.resolveEntityLabel('PRODUCT_CATEGORY')
      )
      ...
      if (error.code === '23505') {
        return res.status(409).json({ success: false, message: 'Danh mục này đã tồn tại trong trại' })
      }
  ...
  async deleteProductCategory(req, res) {
    try {
      const deleted = await productService.deleteProductCategory(req.params.categoryId, req.user.farm_id)
      if (!deleted) {
        return res.status(404).json({ success: false, message: 'Danh mục không tồn tại' })
      }
  ...
  async createProduct(req, res) {
    try {
      ...
      const categoryName = req.body.categoryName || req.body.category_name || req.body.newCategoryName
      ...
      if (!unit || !String(unit).trim()) {
        return res.status(400).json({ success: false, message: 'Đơn vị tính không được để trống' })
      }
      if (Number(unitPrice) < 0 || Number.isNaN(Number(unitPrice))) {
        return res.status(400).json({ success: false, message: 'Giá đơn vị không hợp lệ' })
      }
      ...
      await auditLogService.logActivity(
        ...
        'PRODUCT',
        product?.product_id,
        { productName, categoryId: product?.category_id, unit, supplier, unitPrice, note },
      ...
      if (error.code === '23505') {
        return res.status(409).json({ success: false, message: 'Sản phẩm hoặc mã sản phẩm đã tồn tại' })
      }
      res.status(400).json({ success: false, message: error.message })
    }
  },
