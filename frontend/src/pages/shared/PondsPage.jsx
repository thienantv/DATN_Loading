...
const getPondStatusBadge = (status) => {
  switch (normalizeUpper(status)) {
    case 'CHUAN_BI_NUOI': return <span className="bg-violet-100 text-violet-700 px-3 py-1 rounded-full text-xs font-bold border border-violet-200">Chuẩn bị nuôi</span>;
    case 'DANG_NUOI': return <span className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-xs font-bold border border-emerald-200">Đang nuôi</span>;
    case 'DANG_CAI_TAO': return <span className="bg-amber-100 text-amber-700 px-3 py-1 rounded-full text-xs font-bold border border-amber-200">Đang cải tạo</span>;
    case 'TAM_NGUNG': return <span className="bg-rose-100 text-rose-700 px-3 py-1 rounded-full text-xs font-bold border border-rose-200">Tạm ngưng</span>;
...
const PondsPage = ({ roleLabel = 'Owner' }) => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      // GỌI API XÓA TRỰC TIẾP (Không dùng Optimistic Update vì dễ lỗi khóa ngoại DB)
      await pondService.deletePond(id);
      showToast({ title: 'Xóa thành công', type: 'success' });
      fetchData();
    } catch (err) { 
      // 🌟 Lấy chính xác nguyên nhân lỗi từ Backend trả về
  ...
  return (
    <div className="max-w-[1600px] mx-auto animate-in fade-in duration-300">
      ...
      <div className="relative bg-gradient-to-r from-emerald-50 via-teal-50 to-cyan-50 rounded-[24px] p-6 md:p-8 mb-6 border border-emerald-100/60 shadow-sm overflow-hidden flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        {isOwner && (
          <div className="relative z-10 flex flex-wrap gap-3 w-full md:w-auto">
            ...
            <button onClick={() => { setForm({ pondName: '', area_m2: '', depth_m: '', assigned_staff: '' }); setShowCreateModal(true); }} className="flex-1 md:flex-none px-5 py-2.5 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 shadow-md shadow-emerald-600/20 transition-all flex items-center justify-center gap-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4 md:gap-5 mb-6">
        ...
        <div className="bg-white p-5 rounded-[20px] border border-slate-100 shadow-sm relative overflow-hidden">
          <div className="flex justify-between items-start mb-2">
            <span className="text-slate-500 font-bold text-sm">Đang cải tạo</span>
      ...
      <div className={`grid grid-cols-1 ${isOwner ? 'lg:grid-cols-3' : 'lg:grid-cols-2'} gap-5 mb-6`}>
        ...
        <div className="bg-white p-5 md:p-6 rounded-[24px] border border-slate-100 shadow-sm flex flex-col h-[320px]">
           <h3 className="font-extrabold text-slate-800 text-lg mb-4">Trạng thái ao nuôi</h3>
           <div className="flex-1 flex items-center">
              <div className="w-1/2 h-[180px]">
                {/* 🌟 Fix Lỗi Cảnh Báo Console Recharts: Đổi height="100%" thành height={180} tĩnh */}
        ...
        <div className="bg-white p-5 md:p-6 rounded-[24px] border border-slate-100 shadow-sm flex flex-col h-[320px]">
           <h3 className="font-extrabold text-slate-800 text-lg mb-4">Trạng thái sử dụng</h3>
           <div className="flex-1 flex items-center">
              <div className="w-1/2 h-[180px]">
                <ResponsiveContainer width="100%" height={180}>
                  <PieChart>
        ...
        {isOwner && (
          <div className="bg-white p-5 md:p-6 rounded-[24px] border border-slate-100 shadow-sm flex flex-col h-[320px]">
            <h3 className="font-extrabold text-slate-800 text-lg mb-4">Phân công kỹ sư</h3>
              ...
              {paginatedPonds.length === 0 ? (
                ...
                paginatedPonds.map(pond => (
                  <tr key={pond.pond_id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-center">{getPondStatusBadge(pond.status)}</td>
                        ...
                        {isOwner && (
                          <>
                            ...
                            <button onClick={() => handleToggleUsage(pond)} className={`p-2 rounded-lg border transition-all shadow-sm ${normalizeUpper(pond.usage_status) === 'HOAT_DONG' ? 'bg-white border-slate-200 text-rose-500 hover:bg-rose-50 hover:border-rose-200' : 'bg-emerald-50 border-emerald-200 text-emerald-600 hover:bg-emerald-100'}`} title={normalizeUpper(pond.usage_status) === 'HOAT_DONG' ? 'Tạm ngưng sử dụng' : 'Mở lại hoạt động'}>
                              {normalizeUpper(pond.usage_status) === 'HOAT_DONG' ? (
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 9v6m4-6v6m7-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> // Icon Pause
                              ) : (
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> // Icon Play
                              )}
                            </button>

                            {/* Nút Xóa (Đã Khôi Phục) */}
      ...
      {showDetailModal && selectedPond && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4" onClick={() => setShowDetailModal(false)}>
          <div className="bg-white max-w-2xl w-full p-6 md:p-8 rounded-[24px] shadow-2xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100"><span className="text-xs font-bold text-slate-500 uppercase block mb-1">Tên ao</span><strong className="text-lg text-slate-800">{selectedPond.pond_name}</strong></div>
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100"><span className="text-xs font-bold text-slate-500 uppercase block mb-1">Trạng thái</span><div className="mt-1">{getPondStatusBadge(selectedPond.status)}</div></div>
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100"><span className="text-xs font-bold text-slate-500 uppercase block mb-1">Diện tích</span><strong className="text-lg text-slate-800">{formatRoundedNumber(selectedPond.area_m2)} m²</strong></div>
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100"><span className="text-xs font-bold text-slate-500 uppercase block mb-1">Độ sâu</span><strong className="text-lg text-slate-800">{formatRoundedNumber(selectedPond.depth_m)} m</strong></div>
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 col-span-2 sm:col-span-1"><span className="text-xs font-bold text-slate-500 uppercase block mb-1">Kỹ sư quản lý</span><strong className="text-base text-slate-800">{selectedPond.technician_name || getTechnicianName(selectedPond.assigned_staff) || '-'}</strong></div>
              {isOwner && <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 col-span-2 sm:col-span-1"><span className="text-xs font-bold text-slate-500 uppercase block mb-1">Công nhân trực</span><strong className="text-base text-slate-800">{(selectedPond.workers || []).map(w => w.full_name || w.username).join(', ') || '-'}</strong></div>}
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4" onClick={() => {setShowCreateModal(false); setShowEditModal(false)}}>
          <div className="bg-white max-w-md w-full p-6 md:p-8 rounded-[24px] shadow-2xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={showEditModal ? handleEditSubmit : handleCreateSubmit} className="flex flex-col gap-4">
              ...
              {showEditModal && <div className="flex flex-col gap-1.5"><label className="text-sm font-bold text-slate-700">Tình trạng sử dụng</label><select value={form.usage_status} onChange={(e) => setForm({...form, usage_status: e.target.value})} className="w-full px-4 py-3 border border-slate-300 rounded-xl bg-white outline-none focus:ring-2 focus:ring-emerald-100 focus:border-emerald-500 font-medium"><option value="HOAT_DONG">Hoạt động</option><option value="NGUNG_SU_DUNG">Ngưng sử dụng</option></select></div>}
