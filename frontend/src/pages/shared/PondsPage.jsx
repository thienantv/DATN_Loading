...
const getPondStatusBadge = (status) => {
  switch (normalizeUpper(status)) {
    ...
    case 'DANG_CAI_TAO': return <span className="bg-amber-100 text-amber-700 px-3 py-1 rounded-full text-xs font-bold border border-amber-200 shadow-sm">Đang cải tạo</span>;
...
const PondsPage = ({ roleLabel = 'Owner' }) => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true); // Chỉ hiện loading cục bộ
  ...
  // 🌟 Chỉ hiện Full-screen Loading vào lần tải ĐẦU TIÊN (khi chưa có dữ liệu)
