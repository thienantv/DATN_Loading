...
const OwnerPonds = () => {
    ...
    try {
      ...
      showToast({
        title:
          ...
          'Không tải được dữ liệu phân công công nhân',
        type: 'error',
      })
  ...
  const handleWorkerAssignmentChange = async (technicianId, workerId, checked) => {
    try {
      ...
      // 1. Lọc ra danh sách worker_id hiện tại của kỹ sư này từ mảng assignments gốc
      ...
      let nextWorkerIds
      if (checked) {
        // Nếu chọn thêm: thêm ID vào mảng nếu chưa có
        if (!currentWorkerIds.includes(Number(workerId))) {
          nextWorkerIds = [...currentWorkerIds, Number(workerId)]
        } else {
          nextWorkerIds = currentWorkerIds
        }
      } else {
        // Nếu bỏ chọn: lọc bỏ ID đó đi
        nextWorkerIds = currentWorkerIds.filter((id) => Number(id) !== Number(workerId))
      }

      // 2. GỬI API: Đảm bảo truyền một Object chứa mảng workerIds theo đúng yêu cầu Backend
      // Nếu trong dịch vụ userService bạn chỉ nhận (techId, data) thì gửi như thế này:
      await userService.updateTechnicianWorkerAssignment(technicianId, {
      ...
      showToast({
        title: 'Cập nhật phân công công nhân thành công',
        type: 'success',
      })
    } catch (err) {
      showToast({
        title: err?.response?.data?.message || 'Không thể cập nhật phân công',
        type: 'error',
      })
    } finally {
  ...
  return (
    <div className="dashboard admin-page owner-ponds">
      ...
      {showWorkerAssignmentModal && (
        ...
        >
          <div
            className="modal-content admin-users_modal owner-ponds_assignment-modal"
            onClick={(e) => e.stopPropagation()}
          >
            <h3>
              Phân công công nhân theo kỹ sư phụ trách
            </h3>
              ...
              <table className="table-base owner-ponds_assignment-table">
                <thead>
                  <tr>
                    ...
                    {(workerAssignment.technicians ||
                      []).map((tech) => (
                        <th
                          key={`tech-head-${tech.user_id}`}
                        >
                          {tech.full_name ||
                            tech.username}
                        </th>
                  ...
                  {(workerAssignment.workers ||
                    ...
                    <tr>
                      <td
                        ...
                        className="owner-ponds_assignment-empty"
                      >
                        Chưa có công nhân trong
                        trại nuôi.
                      </td>
                    </tr>
                  ) : (
                      ...
                      []).map((worker) => (
                        ...
                        >
                          <td>
                            ...
                            <div className="owner-ponds_assignment-sub">
                              {worker.username}
                            </div>
                          </td>
                          ...
                          {(workerAssignment.technicians ||
                            []).map((tech) => {
                              const assigned = (workerAssignment.assignments || []).some(
                                (a) =>
                                  String(a.technician_id) === String(tech.user_id) &&
                                  String(a.worker_id) === String(worker.user_id)
                              )

                              return (
                                <td
                                  key={`worker-${worker.user_id}-tech-${tech.user_id}`}
                                >
                                  <input
                                    type="checkbox"
                                    checked={assigned}
